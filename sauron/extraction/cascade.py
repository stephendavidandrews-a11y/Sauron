...
def cascade_object_confirmation(
    conn,
    entity_id: str,
    canonical_name: str,
    original_names: list[str],
    ...
    """5-step cascade for unified_entities (subset of person cascade).

    Skips: voice profile (N/A), NA contact sync (N/A), relational ref detection (N/A).
    """
    stats = {
        "step1_subject_linked": 0,
        "step2_target_linked": 0,
    ...
    # ── Step 1: Subject linking (non-person claims only) ─────────
    try:
        from sauron.api.corrections import sync_claim_entities_subject
        ...
        query = f"""
            ...
            FROM event_claims ec
            WHERE LOWER(TRIM(ec.subject_name)) IN ({placeholders})
              AND ec.subject_type IS NOT NULL AND ec.subject_type != 'person'
              AND ec.review_status IS NOT 'dismissed'
              AND (
                  ec.subject_entity_id IS NULL
                  OR (ec.subject_entity_id != ?
        ...
        for row in rows:
            try:
                sync_claim_entities_subject(
                    conn, row["id"], entity_id, canonical_name, source,
                    entity_table="unified_entities",
                )
                stats["step1_subject_linked"] += 1
                affected_claim_ids.add(row["id"])
            except Exception:
                logger.exception(
                    f"Object cascade Step 1 failed for claim {row['id'][:8]}"
                )
    except Exception:
        logger.exception("Object cascade Step 1 import/query failed")
    ...
    try:
        ...
        for row in rows:
            try:
                ...
                logger.exception(
                    f"Object cascade Step 2 failed for claim_entity {row['id'][:8]}"
                )
    except Exception:
        logger.exception("Object cascade Step 2 query failed")
    ...
    if affected_claim_ids:
        try:
            ...
            logger.exception("Object cascade Step 3 failed")
    ...
    try:
        ...
        logger.exception("Object cascade Step 4 failed")

    # ── Step 5: Alias learning ───────────────────────────────────
    try:
        from sauron.extraction.alias_learner import learn_entity_alias

        for name in clean_names:
            if learn_entity_alias(conn, entity_id, name, canonical_name):
                stats["step5_aliases_learned"] += 1
    except Exception:
        logger.exception("Object cascade Step 5 failed")

    logger.info(
        f"Object cascade for '{canonical_name}': "
        f"S1={stats['step1_subject_linked']} S2={stats['step2_target_linked']} "
        f"S3={stats['step3_text_rewritten']} S4={stats['step4_titles_updated']} "
        f"S5={stats['step5_aliases_learned']}"
    )
