...
## Status
Final draft — **Phase A complete, Phase B complete, Phase B.5 complete, Phase C complete, Phase D complete** (implementation in progress)

## Implementation Progress

### Phase A — Networking App endpoint changes
**STATUS: COMPLETE** (done prior to this implementation session)
- sourceSystem/sourceId fields added to Prisma models
- Upsert logic on POST /api/interactions
- POST /api/standing-offers endpoint created
- POST /api/scheduling-leads endpoint created
- POST /api/contacts/[id]/life-events endpoint created
- POST handler added to /api/commitments
- Prisma migration run

### Phase B — Sauron routing infrastructure
**STATUS: COMPLETE** (2026-03-10)

#### B.1 routing_log table (Section 13.2) — DONE
- Added `routing_log` table to `sauron/db/schema.py` (ROUTING_LOG_SQL constant)
- Added v15 migration to `sauron/db/migrate.py` (_run_v15_routing_log)
- Fixed pre-existing bug: moved `if __name__ == '__main__'` block to end of migrate.py
  (was between run_migration and helper function definitions, causing NameError)
- Migration run successfully — table created with 4 indexes
- Schema: id, conversation_id, target_system, route_type, object_class, status,
  entity_id, attempts, last_attempt_at, last_error, payload_json, networking_item_id

#### B.2 reviewed_payload.py fix (Section 10.7) — DONE
- `sauron/routing/reviewed_payload.py` rewritten
- Now loads `memory_writes` from claims pass (pass 2) extraction JSON
- Now loads `new_contacts_mentioned` from claims pass (pass 2) extraction JSON
- Previously both were initialized as empty lists and never populated
- Verified: test conversation shows 40 memory_writes and 8 new_contacts loaded correctly

#### B.3 Contact ID bridge resolution (Section 11.3) — DONE
- New module: `sauron/routing/contact_bridge.py`
  - `resolve_networking_contact_id(conversation_id)` — finds primary non-Stephen contact,
    returns entity_id + networking_app_contact_id + resolved flag
  - `resolve_entity_networking_id(entity_id)` — direct lookup by entity ID
  - Strategy: queries event_claims → unified_contacts, falls back to transcripts → unified_contacts
  - NO HTTP name-string lookups — purely local DB resolution
- Verified: correctly resolves Stephen Weber → networking_app_contact_id=483e1281...

#### B.4 Pending-entity hold and release (Section 11.4) — DONE
- New module: `sauron/routing/routing_log.py`
  - `log_pending_entity()` — stores routing payload in routing_log with status='pending_entity'
  - `release_pending_routes(entity_id, networking_app_contact_id)` — sends stored payloads
    and sets conversations.routed_at
  - `get_routing_status()` — returns pending/failed/sent counts for Today page
  - `get_failed_routes()` — returns failed routes eligible for retry (max 5 attempts)
  - `log_routing_success()` / `log_routing_failure()` — structured logging

- `sauron/routing/networking.py` rewritten:
  - Uses contact bridge instead of `_find_contact_id()` HTTP name lookups
  - Holds as pending_entity when contact has no networking_app_contact_id
  - Logs all routing outcomes to routing_log
  - Removed `_create_ingestion_item()` — Sauron no longer writes to Networking inbox
  - Memory write functions now accept fallback_contact_id (from bridge resolution)
  - `_resolve_contact_id_for_entity()` replaces `_find_contact_id()` — local DB lookup only

- Release triggers wired into 3 code paths:
  - `sauron/api/graph.py` `confirm_provisional_contact` — after push to Networking App returns ID
  - `sauron/api/graph.py` `link_provisional_to_existing` — after merge into confirmed contact
  - `sauron/contacts/sync.py` `sync_contacts_from_networking_app` — after sync populates IDs

- `sauron/api/conversations.py` `mark_reviewed` updated:
  - No longer unconditionally sets `routed_at`
  - Checks routing_log for pending_entity or failed entries
  - Only sets `routed_at` when no pending/failed routes exist

#### Files changed in Phase B
- `sauron/db/schema.py` — added ROUTING_LOG_SQL, patched init_db()
- `sauron/db/migrate.py` — added _run_v15_routing_log(), fixed __main__ ordering
- `sauron/routing/reviewed_payload.py` — loads memory_writes + new_contacts from pass 2
- `sauron/routing/networking.py` — full rewrite with contact bridge + routing_log
- `sauron/routing/contact_bridge.py` — NEW: contact ID bridge resolution
- `sauron/routing/routing_log.py` — NEW: routing log management + release triggers
- `sauron/api/graph.py` — release triggers in confirm + link provisional
- `sauron/api/conversations.py` — mark_reviewed routed_at gating
- `sauron/contacts/sync.py` — release trigger after sync

### Phase B.5 — Extraction layer updates
**STATUS: COMPLETE** (2026-03-10)

#### B.5.1 SynthesisResult schema update — DONE
- Added `sentiment: str | None` to `SynthesisResult` in `sauron/extraction/schemas.py`
  - Enum: warm | neutral | transactional | tense | enthusiastic
  - Default: None (backward compatible — existing data validates without it)
- Added `relationship_delta: str | None` to `SynthesisResult`
  - Enum: strengthened | maintained | weakened | new
  - Default: None (backward compatible)
- Both fields use `Field(default=None, description=...)` for Pydantic validation

#### B.5.2 Opus synthesis prompt update — DONE
- Added `"sentiment"` and `"relationship_delta"` to the JSON schema in the system prompt
  in `sauron/extraction/deep.py` (SYNTHESIS_SYSTEM_PROMPT)
- Added instructions explaining the enum values and when to use each
- Placed after `word_voice_alignment` in the schema block

#### B.5.3 Networking routing — explicit field preference — DONE
- `_infer_sentiment()` in `sauron/routing/networking.py` now checks `synthesis.get("sentiment")`
  first; only falls back to keyword inference on `relationship_notes` if the explicit
  field is missing or not a valid enum value
- `_infer_delta()` similarly prefers `synthesis.get("relationship_delta")` with keyword fallback
- Fallback keywords expanded: added "enthusiastic"/"transactional" for sentiment,
  "first time"/"new relationship" for delta
- Keyword fallback will be removed once Opus prompt is validated on real conversations

#### B.5.4 Verification — DONE
- Backward compatibility: existing pass 3 JSON without these fields validates (defaults to None)
- All 5 enum values for sentiment accepted
- All 4 enum values for relationship_delta accepted
- Existing DB extraction JSON loads without errors
- Explicit fields preferred over keyword inference
- Keyword inference fallback works when fields absent

#### Files changed in Phase B.5
- `sauron/extraction/schemas.py` — sentiment + relationship_delta on SynthesisResult
- `sauron/extraction/deep.py` — Opus prompt schema + instructions
- `sauron/routing/networking.py` — _infer_sentiment/_infer_delta prefer explicit fields

### Phase C — Rewrite Sauron routing (gap-fill)
**STATUS: COMPLETE** (2026-03-10)

Phase B already rewrote networking.py with contact bridge + routing_log.
Phase C gap-fill addressed remaining Section 10.9 requirements.

#### C.1 Inline commitments with Interaction payload — DONE
- Removed standalone `_create_commitment()` function
- Commitments now sent as `body.commitments` array inside the Interaction POST payload
- Networking App's POST /api/interactions creates Commitment rows inline with interactionId
- Firmness-based routing preserved: social → scheduling_leads, tentative → skip,
  intentional/explicit → inline commitment with `[i_owe]`/`[they_owe]` prefix

#### C.2 Null-only contact field patching — DONE
- `_update_contact_field_null_only()` replaces `_update_contact_field()`
- GETs the contact first, checks if the target field already has a non-null/non-empty value
- Skips update if field is populated (protects user-curated data)
- Preserves array fields (categories, tags) that PUT handler would otherwise wipe
- Field name mapping: employer→organization, job_title→title, email_address→email, etc.

#### C.3 All-or-nothing routing per conversation — DONE
- `route_to_networking_app()` now collects all API call results before logging
- If ANY call fails, logs a single `conversation_bundle` failure with:
  - Full extraction payload stored in payload_json (for retry)
  - Concatenated error summary from all failed calls
- If ALL calls succeed, logs individual successes per object class for audit trail
- Returns `bool` (True = all sent, False = held or failed)
- Safe to re-send: upsert on sourceSystem/sourceId prevents duplicates

#### C.4 Sentiment/relationship_delta — DONE (Phase B.5)
- Already implemented in Phase B.5
- `_infer_sentiment()` and `_infer_delta()` prefer explicit Opus fields,
  fall back to keyword inference on relationship_notes

#### C.5 Pipeline API endpoints — DONE
- `GET /pipeline/routing-status` — returns pending_entity, failed, sent counts
  from routing_log (calls existing `get_routing_status()` in routing_log.py)
- `POST /pipeline/retry-failed-routes` — retries all failed routes under
  MAX_RETRY_ATTEMPTS, deduplicates by conversation_id, runs in background,
  updates routing_log status and conversations.routed_at on success

#### C.6 Removed legacy code — DONE
- `_create_commitment()` removed (inline with interaction)
- `_create_interaction()` / `_create_standing_offer()` etc. replaced by
  unified `_api_call()` helper
- `_create_ingestion_item()` already removed in Phase B

#### Files changed in Phase C
- `sauron/routing/networking.py` — full rewrite with all-or-nothing + inline commitments + null-only patching
- `sauron/api/pipeline_api.py` — added routing-status + retry-failed-routes endpoints

### Phase D — Validate end-to-end
**STATUS: COMPLETE** (2026-03-10)

Phase D addressed code-level gaps found during end-to-end audit.
Manual end-to-end testing with real conversations remains for Phase E.

#### D.1 Missing object class routing — DONE
- **Graph edges → ContactRelationship**: `synthesis.graph_edges` routed to
  `POST /api/contact-relationships`. Both from/to entities must resolve to
  networking_app_contact_ids; unresolved edges skipped. Strength mapped from
  0.0-1.0 float to 1-6 integer scale.
- **Intelligence signals from policy positions**: `synthesis.policy_positions`
  routed to `POST /api/signals` as signalType `policy_position`. Contact
  resolved via bridge. Strength mapped to 0-10 relevanceScore.
- **Referenced resources**: Routing code present but stub — the Opus extraction
  prompt does not yet produce a `referenced_resources` list on SynthesisResult.
  Will activate once extraction layer is updated.
- **New contact stubs**: `claims.new_contacts_mentioned` (name strings) routed
  to `POST /api/contacts` for CRM triage. Dedup check: skips names that already
  resolve to a known unified_contacts entry.

#### D.2 Periodic retry job — DONE
- New module: `sauron/routing/retry.py` — `retry_failed_routes_job()`
  - Loads failed routes from routing_log (under MAX_RETRY_ATTEMPTS)
  - Deduplicates by conversation_id (all-or-nothing)
  - Re-sends stored extraction payload via `route_to_networking_app()`
  - Updates routing_log status + conversations.routed_at on success
- Wired to APScheduler in `main.py` with `IntervalTrigger(minutes=30)`
- Job ID: `retry_failed_routes`, replace_existing=True

#### D.3 router.py bool return — DONE
- `route_extraction()` now captures the bool return from `route_to_networking_app()`
- Logs info on success, warning on failure (with pointer to routing_log)

#### D.4 Frontend routing status — NOTED (not implemented)
- `GET /pipeline/routing-status` API endpoint exists (Phase C)
- Today page does not yet call it — deferred to Phase E frontend work

#### D.5 Object class coverage summary
| Object class | Routing status | Data source |
|---|---|---|
| Interaction + Commitments | Routed (inline) | synthesis + reviewed claims |
| Standing Offer | Routed | synthesis.standing_offers |
| Scheduling Lead | Routed | synthesis.scheduling_leads + social commitments |
| Follow-up | Routed | synthesis.follow_ups |
| Life Event | Routed | claims.memory_writes (field=lifeEvent) |
| Contact field update | Routed (null-only) | claims.memory_writes (entity_type=person) |
| Personal Interest | Routed | claims.memory_writes (field=interest) |
| Personal Activity | Routed | claims.memory_writes (field=activity) |
| Contact Relationship | Routed | synthesis.graph_edges |
| Intelligence Signal | Routed | synthesis.policy_positions |
| Referenced Resource | Stub only | synthesis.referenced_resources (not yet extracted) |
| New Contact stub | Routed (with dedup) | claims.new_contacts_mentioned |

#### Files changed in Phase D
- `sauron/routing/networking.py` — added 4 routing blocks (graph edges, signals, resources, contact stubs)
- `sauron/routing/router.py` — captures bool return, logs result
- `sauron/routing/retry.py` — NEW: periodic retry job module
- `sauron/main.py` — wired retry job to APScheduler (30-min interval)
